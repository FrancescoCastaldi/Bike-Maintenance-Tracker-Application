...
- **Maintenance Records Management:**
  ...
  - Registra azioni rapide (ad esempio cambio gomma, cambio olio) con un click: l'evento viene salvato nella lista principale e annotato anche nel file dedicato `quick_actions.txt`.
...
Per macOS è disponibile lo script `run_gui.command`. Dopo aver reso il file eseguibile (`chmod +x run_gui.command`), è sufficiente un doppio clic oppure l'esecuzione da Terminale:

```bash
./run_gui.command
```

Lo script usa `python3` per creare un ambiente virtuale dedicato (se assente), aggiorna `pip`, installa le dipendenze elencate in `requirements.txt` e infine avvia l'interfaccia grafica del cruscotto futuristico.
...
- Oltre a `records.txt` e `components.txt` la cartella conterrà anche `quick_actions.txt`, ossia il registro cronologico delle scorciatoie inserite dal pannello rapido.
