...
QUICK_ACTIONS_FILE_NAME = "quick_actions.txt"
MAX_RECORDS = 100
...
class DataManager:
    ...
    def __init__(self, data_dir: Path | None = None) -> None:
        ...
        self.quick_actions_file: Path = Path()
    ...
    def log_quick_action(self, action_name: str, date: str, notes: str | None = None) -> None:
        """Append a quick maintenance action to the dedicated log file."""

        self.data_dir.mkdir(parents=True, exist_ok=True)
        log_line = f"{date} | {action_name.strip()}"
        if notes:
            cleaned_notes = notes.replace("\n", " ").strip()
            if cleaned_notes:
                log_line += f" | {cleaned_notes}"
        log_line += "\n"
        with self.quick_actions_file.open("a", encoding="utf-8") as file:
            file.write(log_line)
    ...
    def _update_file_paths(self) -> None:
        ...
        self.quick_actions_file = self.data_dir / QUICK_ACTIONS_FILE_NAME
...
class BikeMaintenanceApp(ttk.Frame):
    def __init__(self, master: Window, manager: DataManager) -> None:
        ...
        self.quick_action_var = tk.StringVar(value="")
        self.quick_action_note_var = tk.StringVar(value="")
    ...
    def _create_quick_entry_panel(self) -> None:
        ...
        ttk.Label(panel, text="Quick Action", style="Neon.TLabel").grid(
        ...
        action_selector = ttk.Combobox(
            panel,
            textvariable=self.quick_action_var,
            values=quick_actions,
            state="readonly",
        )
        action_selector.grid(row=4, column=0, sticky="we", pady=(4, 0))
        if quick_actions:
            self.quick_action_var.set(quick_actions[0])

        ttk.Label(panel, text="Notes", style="Neon.TLabel").grid(
            row=3, column=1, sticky=W, padx=(16, 0), pady=(12, 0)
        )
        action_notes = ttk.Entry(panel, textvariable=self.quick_action_note_var)
        action_notes.grid(row=4, column=1, sticky="we", padx=(16, 0), pady=(4, 0))

        ttk.Button(
            panel,
            text="Log Quick Action",
            bootstyle="info-outline",
            command=self._submit_quick_action,
        ).grid(row=4, column=2, sticky="e", padx=(16, 0), pady=(4, 0))

        storage_frame = ttk.Frame(panel, style="GlassInner.TFrame")
        storage_frame.grid(row=5, column=0, columnspan=3, sticky="ew", pady=(12, 0))
    ...
    def _submit_quick_action(self) -> None:
        if not self.quick_date_entry:
            return
        ...
        date_value = self.quick_date_entry.entry.get().strip()
        action_value = self.quick_action_var.get().strip()
        notes_value = self.quick_action_note_var.get().strip()

        if not action_value:
            self.quick_status_var.set("Select a quick action preset to log.")
            return

        if not date_value:
            self.quick_status_var.set("Choose a date for the quick action.")
            return
        ...
        if notes_value:
            description += f" - {notes_value}"

        new_record = MaintenanceRecord(date=date_value, description=description)
        ...
        try:
            self.manager.log_quick_action(action_value, date_value, notes_value)
        except OSError as exc:
            self.quick_status_var.set(
                f"Record added but quick log failed: {exc}"
            )
            self._refresh_records()
            return

        self.quick_action_note_var.set("")
        self.quick_status_var.set(
            f"Quick action saved and logged to {QUICK_ACTIONS_FILE_NAME}."
        )
        self._refresh_records()
